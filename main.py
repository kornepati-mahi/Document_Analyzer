...
# ====================== PERFECT PDF READER (GeM, PWD, Scanned) ======================
...
st.markdown('<div class="main-header"><h1>Bid Analyser Pro</h1><p>Works perfectly on GeM • PWD • Hindi • Scanned PDFs</p></div>', unsafe_allow_html=True)

# ====================== HELPERS ======================
def clean_text = lambda t: re.sub(r'[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]', '', t)
clean_text = lambda t: re.sub(r'\s+', ' ', _text(t)).strip()
...
def ask_llm(prompt, context="", model="llama3-70b-8192", temp=0.1):
    ...
    messages = [{"role": "system", "content": "You are an expert in Indian tenders."}]
...
if st.session_state.get("summary"):
    ...
    st.markdown("Perfect for GeM • PWD • Hindi • Scanned PDFs")

st.caption("Bid Analyser Pro • pdfplumber + Groq Llama3-70B • 100% Working")
